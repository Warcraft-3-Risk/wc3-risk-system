...
import { TimerEventType } from '../timer/TimerEventType';
...
export class TransportManager {
	...
	private handleAutoLoadOn(transport: Transport) {
		...
		const timedEventManager: EventTimer = EventTimer.getInstance();
		const transportId: TimerEventType = GetHandleId(transport.unit);

		timedEventManager.addEvent(
			new TimerEvent(transportId, AUTO_LOAD_DURATION, false, true, (remainingTime) => {
	...
	private handleAutoLoadOff(transport: Transport) {
		...
		EventTimer.getInstance().stopEvent(transport.unit.__handle);
	}
}
