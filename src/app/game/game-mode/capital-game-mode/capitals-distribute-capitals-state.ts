...
export class CapitalsDistributeCapitalsState extends BaseState<CapitalsData> {
	onEnterState() {
		...
		this.stateData.playerCapitalSelections.forEach((city) => {
			if (city) {
				countriesWithCapitals.add(CityToCountry.get(city));
			}
		});
		debugPrint(
		...
		debugPrint(`Countries without capitals`);
		...
		debugPrint(`Countries without capitals ${validCityCountries.length}`);
		let validUniqueCountries1 = new Set<Country>(validCityCountries);
		debugPrint(`Countries without capitals ${validUniqueCountries1.size}`);

		let validUniqueCountries = new Array<Country>();
		validCityCountries.forEach((country) => {
			if (!validUniqueCountries.includes(country)) {
				validUniqueCountries.push(country);
			}
		});

		ShuffleArray(validUniqueCountries);
		debugPrint(`Countries without capitals ${validUniqueCountries.length}`);

		debugPrint(`Players with capitals: ${Array.from(this.stateData.capitals.keys()).join(', ')}`);
		this.stateData.playerCapitalSelections.forEach((city, player) => {
			debugPrint(`Player ${player} has chosen a capital: ${city}`);
			if (!city) {
				const country = validUniqueCountries.pop();
