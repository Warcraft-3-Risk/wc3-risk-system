...
export function Navbar() {
  ...
  return (
    <nav data-testid="main-navbar" className="sticky top-0 z-50 bg-[#0d1f2d]/95 backdrop-blur border-b border-[--color-border]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link href="/" data-testid="navbar-logo" className="text-[--color-accent] font-bold text-xl tracking-wide">
            Risk Reforged
          </Link>
          ...
          <div className="hidden md:flex items-center gap-1" data-testid="desktop-nav">
            {navLinks.map((link) => (
              <Link
                ...
                className="px-3 py-2 rounded-md text-sm font-medium text-[--color-text-secondary] hover:text-[--color-accent] hover:bg-[--color-surface] transition-colors"
              >
                {link.label}
              </Link>
            ))}
